...
function requireString(value: unknown, field: string): string {
  if (typeof value !== 'string' || value.length === 0) {
    const err = new Error(`Missing or invalid "${field}"`) as Error & { statusCode: number };
    err.statusCode = 400;
    throw err;
  }
  return value;
...
export function registerRoutes(app: FastifyInstance, notes: NoteService): void {
  ...
  app.put('/api/note', async (request) => {
    ...
    if (typeof content !== 'string') {
      const err = new Error('Missing or invalid "content"') as Error & { statusCode: number };
      err.statusCode = 400;
      throw err;
    }
  ...
  app.post('/api/move', async (request) => {
    ...
    const moved = await notes.move(requireString(from, 'from'), requireString(to, 'to'));
    return { path: moved };
  });
