...
export class NoteService {
	...
	private async readDir(logicalDir: string, depth = 0): Promise<TreeNode[]> {
		...
		const absolute =
			logicalDir === "" ? this.root : await this.safeAbsolute(logicalDir);
		let entries: Dirent[];
		try {
		...
		for (const entry of entries) {
			...
			const childPath =
				logicalDir === "" ? entry.name : posix.join(logicalDir, entry.name);
	...
	async writeNote(rawPath: string, content: string): Promise<void> {
		...
		const deltaBytes =
			Buffer.byteLength(content, "utf8") - Number(existing?.size ?? 0);
		const newAncestors = this.quotaDisabled
			? 0
			: await this.countMissingAncestors(absolute);
	...
	async move(rawFrom: string, rawTo: string): Promise<string> {
		...
		const composedDepth =
			toLogical.split("/").length + (await subtreeDepth(fromAbs, MAX_DEPTH));
	...
	async deleteNote(rawPath: string): Promise<void> {
		...
		if (!info?.isFile()) throw new NotFoundError("Note not found");
		await rm(absolute);
	...
	async deleteFolder(rawPath: string): Promise<void> {
		...
		const absolute = await this.safeAbsolute(logicalPath);
		const info = await statOrNull(absolute);
		if (!info?.isDirectory()) throw new NotFoundError("Folder not found");
		const freed = this.quotaDisabled
			? { bytes: 0, entries: 0 }
			: await measureSubtree(absolute);
