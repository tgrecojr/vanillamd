...
/** Remove every code unit at or below U+0020 (C0 controls and space). */
...
export function sanitizeUrl(
	...
	const collapsed = stripControlAndSpace(trimmed);

	const scheme = SCHEME_PATTERN.exec(collapsed);
	if (!scheme) return url; // relative — no scheme to abuse

	// The full match is the captured scheme plus its trailing colon.
	const normalized = scheme[0].toLowerCase();
