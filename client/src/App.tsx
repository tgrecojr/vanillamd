...
export function App(): React.JSX.Element {
  ...
  const rename = useCallback(
    async (node: TreeNode): Promise<void> => {
      ...
      try {
        ...
        await refreshTree();
        if (open && (open.path === node.path || open.path.startsWith(`${node.path}/`))) {
  ...
  const remove = useCallback(
    async (node: TreeNode): Promise<void> => {
      const what = node.type === 'note' ? 'note' : 'folder and everything in it';
  ...
  return (
    <div className="layout">
      ...
      <main className="main">
        ...
        {open ? (
          <Editor key={open.path} initialValue={open.content} onChange={onEditorChange} />
        ) : (
