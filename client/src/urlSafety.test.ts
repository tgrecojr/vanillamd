...
import { SAFE_FALLBACK_URL, sanitizeUrl } from "./urlSafety.js";
...
describe("markdown URL scheme allowlist", () => {
	...
	it("allows data: images but never data: links", () => {
		...
		expect(
			sanitizeUrl("data:image/png;base64,iVBORw0KGgo=", { kind: "image" }),
		).toBe("data:image/png;base64,iVBORw0KGgo=");
		expect(
			sanitizeUrl("data:text/html;base64,PHN2Zz4=", { kind: "image" }),
		).toBe(SAFE_FALLBACK_URL);
	});
